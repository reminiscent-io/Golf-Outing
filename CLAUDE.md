...
| [lib/db](lib/db/) | Drizzle schema + `pg.Pool`. Tables: `users`, `user_handicap_history`, `user_follows`, `user_trip_follows`, `trips`, `players` (incl. `invited_phone`/`claim_code` for phone-tag claiming), `rounds`, `round_group_assignments`, `round_group_completions`, `scores`, `scramble_scores`, `round_kudos`, `round_comments`. |
...
- **`trips.kind` drift (gotcha)**: this doc's personal/event trip model is **not** in the checked-in schema or `api-server/src/` — [trips.ts](lib/db/src/schema/trips.ts) has no `kind` column. The live DB still has `kind` (with data), so a naive `db push` will try to drop it. Reconcile schema ↔ live DB before pushing.
